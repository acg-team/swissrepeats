...
# Frequency
df_archaea_test <- df_archaea %>% 
  select(nTR, Freq, Superkingdom) %>% 
  arrange(Superkingdom) %>% 
  spread(Superkingdom, Freq)
chisq.test(df_archaea_test$`archaeal virus-host`, df_archaea_test$`archaeal viruses`)

# Ratio
df_archaea_test <- df_archaea %>% 
  select(nTR, fraction_nTR, Superkingdom) %>% 
  arrange(Superkingdom) %>% 
  spread(Superkingdom, fraction_nTR)
chisq.test(df_archaea_test$`archaeal virus-host`, df_archaea_test$`archaeal viruses`)

cor.test(df_archaea_test$`archaeal virus-host`, df_archaea_test$`archaeal viruses`, method = "spearman")
```
